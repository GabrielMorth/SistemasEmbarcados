...
// Referencia as finções e variaveis usadas na main
void ligarMotor();
void desligarMotor();
...
// void printMatrizTransicaoEstados();

// Referencia as variaveis usadas e as funções utilizadas na máquina de estados
int estado;
int eventoInterno;
...
void setup() {
  ...
  // printMatrizTransicaoEstados();
}

// Inicia o loop infinito que permite a interação do usuário com a máquina 
void loop() {
  ...
  if (reading >= peso*1.1 || reading <= peso*.9){
    peso = reading;
  }

  if (reading2 >= peso2*1.1 || reading2 <= peso2*.9){
    peso2 = reading2;
  }
